...
std::vector<GameAction> GameView::sane_actions() const {
  std::vector<GameAction> all_actions = legal_actions();
  std::vector<GameAction> actions;
  for (const auto &action : all_actions) {
    bool contains_tech = false;
    bool full = action.size() == rules.get_spell_cap();
    for (const auto &spell_id : action) {
      const Spell &spell = rules.get_spell(spell_id);
      if (spell.is_tech_spell()) contains_tech = true;
    }
    // Actions that have room for another spell and don't upgrade a book are suboptimal
    if (!contains_tech && !full) {
      continue;
    }
    // If the spell hasn't been deemed suboptimal, it's sane
